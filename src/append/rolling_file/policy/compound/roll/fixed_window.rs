...
where
    ...
    Q: AsRef<Path>,
{
    // println!("( {} ) moving {} ({}) to {} ({})", std::process::id(), src.as_ref().display(), std::fs::metadata(&src).map(|m| m.len()).unwrap_or(0), dst.as_ref().display(), std::fs::metadata(&dst).map(|m| m.len()).unwrap_or(0));
    // first try a rename
